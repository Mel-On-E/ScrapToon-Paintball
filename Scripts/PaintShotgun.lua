...
function PaintShotgun.client_onCreate( self )
...
function PaintShotgun.client_onRefresh( self )
	self:loadAnimations()
end

function PaintShotgun.loadAnimations( self )
	...
	self.normalFireMode = {
		...
		spreadCooldown = 0.18,
		spreadIncrement = 2.6,
		spreadMinAngle = 10.0, --Paintball
		spreadMaxAngle = 20.0, --Paintball
		fireVelocity = 130.0,
	...
	self.aimFireMode = {
		...
		spreadCooldown = 0.18,
		spreadIncrement = 1.3,
		spreadMinAngle = 7.5, --Paintball
		spreadMaxAngle = 12.5, --Paintball
		fireVelocity =  130.0,
...
function PaintShotgun.client_onUpdate( self, dt )
	--Paintball
...
function PaintShotgun.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function PaintShotgun.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function PaintShotgun.onAim( self, aiming )
	self.aiming = aiming
...
function PaintShotgun.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function PaintShotgun.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function PaintShotgun.onShoot( self, dir )
...
function PaintShotgun.calculateFirePosition( self )
...
function PaintShotgun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function PaintShotgun.calculateFpMuzzlePos( self )
...
function PaintShotgun.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not g_cl_gameManager or g_cl_gameManager:cl_spendPaint(self.data.paintCost) then--Paintball
			...
			--dir = sm.noise.gunSpread( dir, spreadDeg ) --Paintball
...
function PaintShotgun.cl_onSecondaryUse( self, state )
...
function PaintShotgun.client_onEquippedUpdate( self, primaryState, secondaryState )
