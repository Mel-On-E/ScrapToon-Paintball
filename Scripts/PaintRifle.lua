...
PaintRifle = class(PaintGun)
 --Paintball

local renderables = {
	"$CONTENT_DATA/Objects/Char_spudgun/Base/char_spudgun_base_basic.rend", --Paintball
	...
	"$CONTENT_DATA/Objects/Char_spudgun/Tank/Tank_basic/char_spudgun_tank_basic.rend" --Paintball
}
...
function PaintRifle.client_onCreate(self)
...
function PaintRifle.client_onRefresh(self)
	self:loadAnimations()
end

function PaintRifle.loadAnimations(self)
	self.tpAnimations =
		createTpAnimations(
		self.tool,
		{
...
function PaintRifle.client_onUpdate(self, dt)
	has_updated = true
	--Paintball
	...
	if self.tool:isLocal() then
		if self.equipped then
			if
				isSprinting and self.fpAnimations.currentAnimation ~= "sprintInto" and
					self.fpAnimations.currentAnimation ~= "sprintIdle"
			 then
				swapFpAnimation(self.fpAnimations, "sprintExit", "sprintInto", 0.0)
			elseif
				not self.tool:isSprinting() and
					(self.fpAnimations.currentAnimation == "sprintIdle" or self.fpAnimations.currentAnimation == "sprintInto")
			 then
	...
	if self.tool:isLocal() then
		...
		local recoilDispersion =
			1.0 - (math.max(fireMode.minDispersionCrouching, fireMode.minDispersionStanding) + fireMode.maxMovementDispersion)
		...
		local spreadFactor =
			fireMode.spreadCooldown > 0.0 and clamp(self.spreadCooldownTimer / fireMode.spreadCooldown, 0.0, 1.0) or 0.0
	...
	if
		(((isAnyOf(self.tpAnimations.currentAnimation, {"aimInto", "aim", "shoot"}) and
			(relativeMoveDirection:length() > 0 or isCrouching)) or
			(self.aiming and (relativeMoveDirection:length() > 0 or isCrouching))) and
			not isSprinting)
	 then
	...
	self.tool:updateJoint(
		"jnt_hips",
		sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
		0.35 * finalJointWeight * (normalWeight)
	)
	...
	self.tool:updateJoint(
		"jnt_spine1",
		sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
		(0.10 + crouchSpineWeight) * finalJointWeight
	)
	self.tool:updateJoint(
		"jnt_spine2",
		sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
	...
	self.tool:updateJoint(
		"jnt_spine3",
		sm.vec3.new(totalOffsetX, totalOffsetY, totalOffsetZ),
		(0.45 + crouchSpineWeight) * finalJointWeight
	)
...
function PaintRifle.sv_n_onAim(self, aiming)
	self.network:sendToClients("cl_n_onAim", aiming)
end

function PaintRifle.cl_n_onAim(self, aiming)
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim(aiming)
	end
end

function PaintRifle.onAim(self, aiming)
	self.aiming = aiming
	if
		self.tpAnimations.currentAnimation == "idle" or self.tpAnimations.currentAnimation == "aim" or
			self.tpAnimations.currentAnimation == "relax" and self.aiming
	 then
		setTpAnimation(self.tpAnimations, self.aiming and "aim" or "idle", 5.0)
	end
end

function PaintRifle.sv_n_onShoot(self, dir)
	self.network:sendToClients("cl_n_onShoot", dir)
end

function PaintRifle.cl_n_onShoot(self, dir)
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot(dir)
	end
end

function PaintRifle.onShoot(self, dir)
...
function PaintRifle.calculateFirePosition(self)
	local crouching = self.tool:isCrouching()
	local firstPerson = self.tool:isInFirstPersonView()
	local dir = sm.localPlayer.getDirection() + sm.vec3.new(0.25, 0.25, 0.25)
...
function PaintRifle.calculateTpMuzzlePos(self)
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin(dir.z)
	local right = sm.localPlayer.getRight()
...
function PaintRifle.calculateFpMuzzlePos(self)
...
function PaintRifle.cl_onPrimaryUse(self, state)
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		if not g_cl_gameManager or g_cl_gameManager:cl_spendPaint(self.data.paintCost) then --Paintball
			...
			if owner then
				...
				self.network:sendToServer("sv_fire_ball", {pos = sm.localPlayer.getPlayer():getCharacter():getWorldPosition() + sm.vec3.new(0,0,0.5), dir = dir * fireMode.fireVelocity, dmg = Damage})
			 --PaintBall
			end
...
function PaintRifle.cl_onSecondaryUse(self, state)
...
function PaintRifle.client_onEquippedUpdate(self, primaryState, secondaryState)
