...
function PaintGatling.client_onCreate( self )
...
function PaintGatling.client_onRefresh( self )
	self:loadAnimations()
end

function PaintGatling.loadAnimations( self )
...
function PaintGatling.client_onUpdate( self, dt )
	--Paintball
...
function PaintGatling.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function PaintGatling.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function PaintGatling.onAim( self, aiming )
	self.aiming = aiming
...
function PaintGatling.sv_n_onShoot( self, dir ) 
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function PaintGatling.cl_n_onShoot( self, dir ) 
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function PaintGatling.onShoot( self, dir ) 
...
function PaintGatling.calculateFirePosition( self )
...
function PaintGatling.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )		
	local right = sm.localPlayer.getRight()
...
function PaintGatling.calculateFpMuzzlePos( self )
...
function PaintGatling.cl_updateGatling( self, dt )
...
function PaintGatling.cl_fire( self )
	...
	if not g_cl_gameManager or g_cl_gameManager:cl_spendPaint(self.data.paintCost) then--Paintball
		...
		if owner then
			...
			self.network:sendToServer("sv_fire_ball", {pos = sm.localPlayer.getPlayer():getCharacter():getWorldPosition() + sm.vec3.new(0,0,0.5), dir = dir * fireMode.fireVelocity, dmg = Damage})--PaintBall
		end
...
function PaintGatling.cl_onSecondaryUse( self, state )
...
function PaintGatling.client_onEquippedUpdate( self, primaryState, secondaryState )
